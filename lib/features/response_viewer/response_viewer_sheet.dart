...
List<TextSpan> _buildHighlightSpans(
    List<Node> nodes, Map<String, TextStyle> theme) {
...
List<List<TextSpan>> _splitSpansByNewline(
    List<TextSpan> spans, TextStyle baseStyle) {
...
class _SoftWrapHighlightView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      // Text.rich (not RichText) so that a parent SelectableRegion can make
...
class _PrettyTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (searchQuery.trim().isEmpty) {
      if (softWrap) {
        // Per-line rendering so wrapped lines keep their number aligned.
        // Also satisfies Bug 2: Text.rich inside SelectableRegion is selectable.
        final normalized = prettyBody.replaceAll('\t', ' ' * _tabSize);
        final parsed = highlight.parse(normalized, language: language);
        final nodes = parsed.nodes;
        final rootSpans = nodes == null
            ? <TextSpan>[]
            : _buildHighlightSpans(nodes, theme);
        ...
        return SelectableRegion(
          ...
          child: ColoredBox(
            color: theme[_SoftWrapHighlightView._rootKey]?.backgroundColor ??
      ...
      // softWrap = false: single widget + horizontal scroll. Wrap in
      // SelectableRegion (Text.rich inside _SoftWrapHighlightView is
      // selectable unlike the old RichText).
...
class _RawTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (searchQuery.trim().isEmpty) {
      // Per-line widgets so each line's number stays aligned when wrapped.
      // SelectableRegion wraps the entire body for cross-line text selection.
...
class _SearchHighlightedScrollBody extends StatelessWidget {
  ...
  static List<InlineSpan> _spansForLine(
      String line, String q, Color highlightBg) {
  ...
  Widget build(BuildContext context) {
    ...
    if (softWrap) {
      // Per-line rendering to keep line numbers aligned under soft-wrap.
    ...
    // softWrap = false: single widget with horizontal scroll.
    final spans = <InlineSpan>[];
    if (q.isEmpty) {
